...
သုရိယ is a Burmese-first, mobile-first Vedic astrology PWA. It calculates a deterministic sidereal birth chart from exact birth details, turns the chart into practical daily guidance, and can stream a careful Burmese interpretation through Gemini.
...
- Gemini streaming behind a server-only provider boundary
- Deterministic local reading fallback when Gemini is not configured
...
Copy `.env.example` to `.env.local` when testing Gemini locally:
...
Never expose `GEMINI_API_KEY` to browser code. When the key is absent, the provider returns a clearly bounded deterministic sample interpretation so the rest of the flow remains testable.
...
- The Gemini key and full prompt stay server-side.
...
The build packages `.openai/hosting.json` and `drizzle/` into `dist/` for Sites hosting. Configure `GEMINI_API_KEY` and optionally `GEMINI_MODEL` as server runtime values in the hosting environment.
