...
export function Dial({ score, label, size = 96 }: { score: number; label: string; size?: number }) {
  ...
  return (
    <svg className="dial" width={size} height={size} viewBox="0 0 100 100" role="img" aria-label={`${label} ${clamped} အမှတ်`}>
      ...
      <text x="50" y="80" textAnchor="middle" className="dial-value">{clamped}</text>
    </svg>
  );
}
